...
function mountainSceneHTML(quincenas, cumbreLograda, height) {
  ...
  return (
    ...
    "</svg>" +
    '<div class="mountain-caption">' + doneCount + " di 6 quindicine conquistate</div>" +
    "</div>"
  );
}
...
function quincenaAscensoHTML(doneCount) {
  ...
  return (
    ...
    '<div class="ascenso-caption">Sei a ' + doneCount + " di " + total + " quindicine conquistate</div>" +
    "</div>"
  );
}
...
function heroMountainHTML(overlayHtml) {
  ...
  return (
    ...
    '<div class="hm-hint">' + Icon("sparkles", { size: 11, color: "rgba(255,255,255,.85)" }) + " Muovi il cursore per svelare la cima</div>" +
    (overlayHtml || "") +
    "</div>"
  );
}
...
/* ---------------------------------------------------------------
   ...
   uno por rango:
     0 Consumatore VIP → sfondo blu "brumoso" + titolo impilato
...
function rankCardNameFontSize(nombre) {
  const len = (nombre || "Il tuo nome").length;
  if (len <= 13) return 92;
  if (len <= 18) return 76;
  if (len <= 24) return 62;
  return 50;
...
function rankCardTemplateSVGMarkup(nombre, photoHref, template) {
  const name = safeXml(nombre || "Il tuo nome");
...
function cardFooterSVG(cx, y) {
  return (
    ...
    '<text x="' + cx + '" y="' + y + '" text-anchor="middle" font-family="Arial, sans-serif" font-size="15" fill="rgba(255,255,255,0.55)">Percorso verso il successo con Atomy</text>'
  );
}
