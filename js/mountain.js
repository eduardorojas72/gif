...
function mountainSceneHTML(quincenas, cumbreLograda, height) {
  ...
  /* Os 6 círculos ficam dourados e mostram um check assim que essa
     quinzena é concluída (quincenas[i+1] true) — o "acendimento" já
     acontece automaticamente aqui. */
  ...
  return (
    ...
    "</svg>" +
    '<div class="mountain-caption">' + doneCount + " de 6 quinzenas conquistadas</div>" +
    "</div>"
  );
}

/* Cartão de cada quinzena individual: foto da montanha + um ponto que sobe
   mais alto quantas mais quinzenas você tiver concluído no total (0 de 6 =
   quase na base, 6 de 6 = quase no topo). É o mesmo "você está aqui" nas 6
   páginas de quinzena, não algo diferente em cada uma. */
function quincenaAscensoHTML(doneCount) {
  ...
  return (
    ...
    '<div class="ascenso-caption">Você está em ' + doneCount + " de " + total + " quinzenas conquistadas</div>" +
    "</div>"
  );
}
...
function heroMountainHTML(overlayHtml) {
  ...
  return (
    ...
    '<div class="hm-hint">' + Icon("sparkles", { size: 11, color: "rgba(255,255,255,.85)" }) + " Mova o cursor para revelar o topo</div>" +
    (overlayHtml || "") +
    "</div>"
  );
}

/* Mini cena de montanha noturna (céu azul + lua + estrelas + silhueta
   de picos) — usada como fundo compartilhado nos cartões das 6 quinzenas
   do Plano 90 e nas insígnias de "6 conquistas" de Conquistas. */
...
/* ---------------------------------------------------------------
   CARTÕES DE RANK COM FOTO REAL — modelos desenhados pela equipe
   (img/*.png) que substituem o design desenhado à mão para os ranks que já
   têm foto própria. Cada modelo traz sua própria faixa com um "Nome" de
   modelo; ela é coberta com um remendo da mesma cor (dourado ou a cor
   própria da faixa, ver maskColor/maskColorLight) antes de escrever por
   cima o nome real do sócio, em negrito e seguindo a curva dessa faixa
   (arch: o quanto o nome se curva para cima no centro). O índice deste
   array corresponde a RANGOS (data.js). */
...
function rankCardTemplateSVGMarkup(nombre, photoHref, template) {
  const name = safeXml(nombre || "Seu nome");
...
function cardFooterSVG(cx, y) {
  return (
    ...
    '<text x="' + cx + '" y="' + y + '" text-anchor="middle" font-family="Arial, sans-serif" font-size="15" fill="rgba(255,255,255,0.55)">Jornada rumo ao sucesso com a Atomy</text>'
  );
}
