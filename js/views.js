...
const MENU_ITEMS = [
  ...
  { id: "socios", label: "Mis Socios", icon: "users" },
...
// Orden del menú lateral de escritorio, en 2 columnas: la primera agrupa el
// recorrido del negocio (entrenamiento y medios), la segunda el resto
// (perfil, contactos/equipo y ajustes). El menú móvil (menú sheet) usa
...
const TOUR_PASOS = [
  ...
  { icon: "users", titulo: "Mis Socios", texto: "Tu directorio permanente de equipo por línea izquierda y derecha — toca un nombre para ver su ID, Zoom, PVP y fecha de última compra. La línea se pone naranja cuando lleva 11 meses sin comprar." },
...
function renderPasos(state, ui) {
  ...
  return header +
    videoRowHTML({ titulo: "Los 8 Pasos Al Éxito - Han Gill Park", url: "https://www.youtube.com/watch?v=z-Nzz1HkoE8" }) +
...
function renderAjustes(state, ui) {
  ...
  return (
    ...
    '<p class="muted small" style="margin-top:6px;line-height:1.5">Todos tus datos (Lista de 250, Árbol Genealógico, tu progreso) viven solo en este dispositivo. Descarga un respaldo y guárdalo donde quieras (tu Google Drive, correo, etc.) — así no lo pierdes si cambias de celular o borras datos del navegador.</p>' +
    '<div class="row gap-2" style="margin-top:10px">' +
...
function renderContactos(state, ui) {
  ...
  const rows = sorted.length
    ? sorted.map(function (c) {
        ...
        return nivelHeader + (
          ...
          '<button class="btn-secondary" style="flex:1;padding:8px;font-size:12.5px" data-action="registrar-contacto" data-arg="' + c.id + '" data-tipo="llamada">' + Icon("phone-call", { size: 13 }) + " Llamada</button>" +
          '<button class="btn-secondary" style="flex:1;padding:8px;font-size:12.5px" data-action="registrar-contacto" data-arg="' + c.id + '" data-tipo="mensaje">' + Icon("message-circle", { size: 13 }) + " Mensaje</button>" +
          "</div>" +
          "</div>"
        );
      }).join("")
  ...
  return (
    ...
    '<p class="muted small" style="margin-top:-4px">Toca “Llamada” o “Mensaje” en cada contacto para que quede registrado en tu Informe Semanal.</p>' +
    ...
    '<div class="row gap-2">' +
    '<button class="btn-primary" style="flex:1" data-action="add-contacto">' + Icon("phone-call", { size: 16, color: "#fff" }) + " Nuevo</button>" +
    '<button class="btn-secondary" style="flex:1" data-action="importar-contactos">' + Icon("download", { size: 16 }) + " Importar contactos</button>" +
    "</div>" +
...
function renderImportarContactosModal(ui) {
  ...
  return (
    ...
    '<div class="row between"><span style="font-weight:700;font-size:15px">Importar contactos</span>' +
    '<button class="icon-btn" data-action="cerrar-importar-contactos">' + Icon("x", { size: 18 }) + "</button></div>" +
    '<p class="muted small" style="margin-top:6px;line-height:1.5">Tu navegador no permite abrir el selector de contactos directamente aquí. Copia tus contactos desde el teléfono — uno por línea, con el formato <b>Nombre, Teléfono</b> — y pégalos abajo:</p>' +
    '<textarea rows="9" id="importar-contactos-textarea" placeholder="Ana Pérez, +34 600 111 222\nCarlos Gómez, +34 600 333 444" style="margin-top:10px;width:100%;background:var(--bg);border:1px solid var(--border);color:var(--text);border-radius:10px;padding:9px 11px;font-size:13px;outline:none;resize:vertical;font-family:inherit"></textarea>' +
...
function renderArbolGenealogico(state, ui) {
  ...
  return (
    ...
    '<div class="field"><label>Fecha de nacimiento</label><input type="date" data-field="arbolGenealogico.yo.fechaNacimiento" value="' + escapeHtml(yo.fechaNacimiento || "") + '"></div>' +
    "</div></div>" +
    ...
    '<div class="field"><label>Grupo de WhatsApp de soporte</label><input type="text" data-field="arbolGenealogico.patrocinador.grupoWhatsapp" value="' + escapeHtml(p.grupoWhatsapp || "") + '" placeholder="Pega aquí el enlace de invitación al grupo"></div>' +
    '<p class="muted small" style="line-height:1.4;margin-top:-4px">Si lo agregas, aparecerá un botón flotante para que tus nuevos socios puedan escribir ahí cuando tengan dudas fuera de tu horario o de la hora de control acordada contigo.</p>' +
    "</div></div>" +
...
function renderSOSModal(ui) {
  ...
  return (
    ...
    '<div class="field"><label>Nombre</label><input type="text" data-draft-field="nombre" value="' + escapeHtml(d.nombre) + '" placeholder="Nombre completo"></div>' +
    '<div class="field"><label>Teléfono</label><input type="text" inputmode="tel" data-draft-field="telefono" value="' + escapeHtml(d.telefono) + '" placeholder="+57 300 000 0000"></div>' +
    '<div class="field"><label>Código/ID de Zoom</label><input type="text" data-draft-field="zoomId" value="' + escapeHtml(d.zoomId || "") + '" placeholder="Ej. 123 456 7890"></div>' +
...
/* ---------------- Mis Socios — directorio permanente por línea izquierda/derecha ---------------- */
...
function misSocioRowHTML(linea, s) {
  ...
  return (
    ...
    '<span style="font-weight:700;font-size:13.5px;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap' + (porVencer ? ";color:var(--warn)" : "") + '">' + escapeHtml(s.nombre || "Sin nombre") + "</span>" +
...
function misSociosColumnaHTML(state, linea) {
  ...
  const rows = sorted.length
    ...
    : '<p class="muted small" style="text-align:center;padding:12px 0">Sin socios aún.</p>';
  return (
    '<div style="min-width:0">' +
    '<div style="font-weight:700;font-size:13px;margin-bottom:6px">' + (linea === "izquierda" ? "Izquierda" : "Derecha") + " · " + lista.length + "</div>" +
    '<button class="btn-secondary" style="width:100%;padding:8px;font-size:12.5px" data-action="add-socio" data-arg="' + linea + '">' + Icon("phone-call", { size: 14 }) + " Agregar</button>" +
    '<div class="view-stack gap-sm" style="margin-top:8px">' + rows + "</div>" +
    "</div>"
  );
}
...
function renderMisSocios(state) {
  return (
    sectionHeaderHTML("Mis Socios", "Tu directorio permanente de equipo, por línea izquierda y derecha. Toca un nombre para ver o actualizar sus datos.", "users") +
    '<div class="card"><p class="small" style="line-height:1.6">Cuando un socio lleve <b>11 meses sin comprar</b>, su nombre se resalta en naranja para avisarte que está a punto de caducar y debe hacer una compra.</p></div>' +
...
function renderSocioModal(ui) {
  ...
  const deleteBtn = editing
    ? '<button class="btn-secondary" style="margin-top:8px;border-color:var(--warn);color:var(--warn)" data-action="delete-socio" data-arg="' + d.id + '">' +
      (ui.confirmDeleteSocio === d.id ? "¿Seguro? Toca de nuevo para eliminar" : "Eliminar socio") +
      "</button>"
    : "";
  const meses = mesesDesde(d.fechaUltimaCompra);
  const aviso = meses !== null && meses >= 11
    ? '<p class="small" style="margin-top:-4px;color:var(--warn);font-weight:600;display:flex;align-items:center;gap:5px">' + Icon("triangle-alert", { size: 13, color: "var(--warn)" }) + "Lleva " + meses + " meses sin comprar — está por caducar</p>"
    : "";
  return (
    '<div class="modal-overlay">' +
    '<div class="modal-backdrop" data-action="cancel-socio"></div>' +
    '<div class="modal-card" style="text-align:left;align-items:stretch;max-width:380px">' +
    '<div class="row between"><span style="font-weight:700;font-size:15px">' + (editing ? "Editar socio" : "Nuevo socio — línea " + (d.linea === "izquierda" ? "Izquierda" : "Derecha")) + "</span>" +
    ...
    '<div class="field"><label>Teléfono</label><input type="text" inputmode="tel" data-draft-field="telefono" value="' + escapeHtml(d.telefono) + '" placeholder="+57 300 000 0000"></div>' +
    '<div class="field"><label>Código/ID de Zoom</label><input type="text" data-draft-field="zoomId" value="' + escapeHtml(d.zoomId || "") + '" placeholder="Ej. 123 456 7890"></div>' +
    '<div class="field"><label>PVP <span class="muted" style="font-weight:400">(actualízalo cuando te lo envíe)</span></label><input type="number" min="0" step="10000" data-draft-field="pvp" value="' + (Number(d.pvp) || 0) + '"></div>' +
    '<div class="row gap-2">' +
    '<div class="field" style="flex:1"><label>Fecha de cumpleaños</label><input type="date" data-draft-field="fechaCumpleanos" value="' + (d.fechaCumpleanos || "") + '"></div>' +
    '<div class="field" style="flex:1"><label>Fecha de última compra</label><input type="date" data-draft-field="fechaUltimaCompra" value="' + (d.fechaUltimaCompra || "") + '"></div>' +
    "</div>" +
    aviso +
    '<div class="field"><label>Notas</label><textarea rows="2" data-draft-field="notas" placeholder="Observaciones...">' + escapeHtml(d.notas || "") + "</textarea></div>" +
    "</div>" +
    '<button class="btn-primary" style="margin-top:14px" data-action="save-socio">Guardar</button>' +
    deleteBtn +
