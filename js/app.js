...
function waHref(numero) {
  return "https://wa.me/" + (numero || "").replace(/[^0-9]/g, "") + "?text=" + encodeURIComponent("Bonjour, j'ai une question sur mon parcours dans Cumbre 90");
}

function waHrefPersonal(numero, nombre) {
  return "https://wa.me/" + (numero || "").replace(/[^0-9]/g, "") + "?text=" + encodeURIComponent("Bonjour" + (nombre ? " " + nombre : "") + " ! Comment vas-tu ?");
}

function shareTextForLogro(titulo) {
  return "🏆 J'ai obtenu le succès \"" + titulo + "\" dans mon parcours vers Sales Master avec Atomy ! 🚀 Si tu es curieux, demande-moi de quoi il s'agit.";
}
...
const App = {
  ...
  checkAgendaAlarmas() {
    ...
    (dia.actividades || []).forEach((a) => {
      ...
      revisar(a, "Cumbre 90 — " + tipo.label, a.nota || "C'est prévu à " + a.hora + ".");
    });
    (dia.zooms || []).forEach((z) => {
      revisar(z, "Cumbre 90 — Zoom : " + (z.titulo || "Réunion"), "Ça commence à " + z.hora + ".");
    });
  },
  ...
  notifyReminders() {
    ...
    try {
      ...
      new Notification("Cumbre 90 — Rappel", {
        body: reminders.length > 1 ? primero.text + " (+" + (reminders.length - 1) + " autres)" : primero.text,
      });
    } catch (e) {
      /* certains navigateurs restreignent Notification hors d'un geste utilisateur : ignoré */
    }
  },
      ...
      if (el.type === "file" && el.dataset && el.dataset.target === "__importBackup") {
        ...
        reader.onload = () => {
          ...
          try {
            ...
            this.showToast("Ce fichier n'est pas une sauvegarde valide de Cumbre 90.");
            return;
          }
          if (!window.confirm("Cela remplacera toutes tes données actuelles (Liste de 250, Arbre Généalogique, progression) par celles du fichier de sauvegarde. Continuer ?")) return;
...
const Actions = {
  ...
  "tour-saltar": function () {
    ...
    App.state.tourVisto = true;
    App.persist(true);
    App.showToast("Tu peux revoir le parcours avec le bouton flottant");
    App.render();
  },
  ...
  "salir-app": function () {
    ...
    App.render();
    window.close();
    App.showToast("Si elle ne s'est pas fermée toute seule, tu peux fermer cet onglet ou revenir en arrière.");
  },
  ...
  "finish-semana": function (arg) {
    ...
    App.addActividad("Tu as terminé la Semaine " + weekN + " (" + semana.paso + ")");
    App.celebrate();
    ...
    if (quincenaCompleta) {
      App.addActividad("Tu as conquis le Camp : " + q.nombre);
      const premio = App.state.premios[q.n - 1];
      let sub = "Camp du Plan de 90 Jours conquis.";
      if (premio) sub += " Tu as débloqué la récompense : " + premio.premio + ".";
      ...
      if (totalCompletas === QUINCENAS.length && !App.state.codigoCumbre) {
        ...
        App.ui.logro = { titulo: "Cumbre 90 — Sales Master", sub: "Tu as terminé les 6 quinzaines du Plan de 90 Jours !", tipo: "cumbre" };
      }
    }
    App.persist(true);
    App.render();
  ...
  "set-rango": function (arg) {
    ...
    if (avanza) {
      App.celebrate();
      App.ui.logro = { titulo: RANGOS[i].nombre, sub: "Nouveau rang atteint chez Atomy.", tipo: "rango", rangoIndex: i };
      App.addActividad("Tu as atteint le rang : " + RANGOS[i].nombre);
    }
    App.persist(true);
    App.render();
  },
  ...
  "compartir-imagen-descargar": function () {
    ...
    App.showToast("Image téléchargée — tu peux maintenant la joindre où tu veux la partager.");
    App.ui.compartirImagenDraft = null;
    App.render();
  },
  ...
  "compartir-imagen-mas-opciones": function () {
    ...
    if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
      navigator.share({ files: [file], title: "Cumbre 90", text: d.shareText }).catch(function (err) {
        ...
        downloadBlob(d.blob, d.filename);
        App.showToast("Impossible d'ouvrir le panneau de partage — l'image a été téléchargée.");
      });
    } else {
      downloadBlob(d.blob, d.filename);
      App.showToast("Image téléchargée — tu peux maintenant la joindre où tu veux la partager.");
  ...
  "compartir-imagen-plataforma": function (arg) {
    ...
    App.showToast("Image téléchargée — ouvre " + (labels[arg] || arg) + " et joins-la là-bas.");
    App.ui.compartirImagenDraft = null;
    App.render();
  },
  ...
  "toggle-notif": function () {
    ...
    if (Notification.permission === "granted") {
      ...
      App.showToast("Active les autorisations de notification dans les réglages de ton navigateur.");
    }
  },
  ...
  "marcar-lema-hoy": function () {
    ...
    if (foco.racha === 7) {
      App.showToast("1 semaine d'affilée à vivre « " + pilar.t + " » ! 🔥");
    } else if (foco.racha === 21) {
      App.celebrate();
      App.ui.logro = { titulo: pilar.t, sub: "21 jours d'affilée — c'est déjà une habitude.", tipo: "generic" };
      App.addActividad("Tu as fait de « " + pilar.t + " » une habitude de 21 jours.");
    }
    App.persist(true);
    App.render();
  },
  ...
  "share-logro-plataforma": function (arg) {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(text).then(
        () => App.showToast("Texte copié — colle-le dans ta story ou ta publication."),
  ...
  "add-contacto": function () {
    App.ui.contactoDraft = { nombre: "", telefono: "", pais: "", nivel: "Tiède", estado: "À contacter", notas: "", notaSeguimiento: "", proximoSeguimiento: null };
  ...
  "save-contacto": function () {
    ...
    const quedoComoSocio = d.estado === "Partenaire" && estadoAnterior !== "Partenaire";
    App.ui.contactoDraft = null;
    App.ui.contactoEditId = null;
    App.persist(true);
    ...
    for (let i = 0; i < 6; i++) {
      ...
      App.state.agenda[weekdayId].actividades.push(
        Object.assign(nuevaActividadAgenda(), {
          tipo: "plan6",
          hora: hora,
          nota: "Jour " + (i + 1) + " — " + diaInfo.titulo + " — avec " + d.contactoNombre,
          recordar: !!hora,
          recordarMin: 10,
        })
      );
  ...
  "share-paso-reflexion": function (arg) {
    ...
    if (navigator.share) {
      ...
      navigator.clipboard.writeText(texto).then(
        () => App.showToast("Réflexion copiée — colle-la où tu veux !"),
        () => App.showToast("Impossible de copier le texte")
      );
    } else {
      App.showToast("Impossible de copier le texte");
  ...
  "set-escenario-avance": function (arg, el) {
    ...
    if (completo && !App.state.escenarioCompletado) {
      ...
      App.celebrate();
      App.ui.logro = { titulo: "Scénario de Vie", sub: "Tu as relié les 8 points en un cercle parfait — tu sais maintenant clairement ton « pourquoi ».", tipo: "generic" };
      App.addActividad("Tu as complété ton Scénario de Vie — cercle parfait !");
    } else if (!completo) {
