    ...
    document.getElementById("fab-slot").innerHTML = isAuth
      ? '<button class="fab-whatsapp" data-action="abrir-whatsapp-fab" title="Escribir a tu patrocinador/a">' + Icon("message-circle", { size: 24, color: "#fff" }) + "</button>" +
        (!ui.tourAbierto
          ...
          : "") +
        (hayCumpleanosHoy
          ? '<button class="fab-birthday" data-action="open-cumpleanos" title="Compleanni di oggi">' + Icon("party", { size: 20, color: "#fff" }) + "</button>"
          : "") +
        (grupoWhatsapp && grupoWhatsapp.trim()
          ? '<button class="fab-whatsapp-group" data-action="abrir-whatsapp-grupo-fab" title="Gruppo WhatsApp di supporto">' + Icon("users", { size: 20, color: "#fff" }) + "</button>"
          : "")
      : "";
  ...
  bindEvents() {
    ...
    root.addEventListener("input", (e) => {
      ...
      if (el.dataset && el.dataset.field) {
        ...
        // formulari con bozza (contatto / attività agenda / zoom / persona Riunione di Allineamento / ascendente / S.O.S. / contatto evento / sponsor dal FAB / socio / cliente): non re-renderizzano, per non perdere il focus
        ...
        // filtro di ricerca dei clienti: si applica direttamente al DOM, senza passare per render()
        const q = el.value.trim().toLowerCase();
...
// Aggiunge nuovi contatti alla Lista dei 250 a partire da {nombre, telefono},
// senza duplicare per telefono (usato sia dal selettore nativo sia dal
// modal "incolla lista" di "Importa contatti").
function importarContactosDesdeListado(entradas) {
  ...
  entradas.forEach(function (e) {
    ...
    App.state.contactos.push({
      ...
      nombre: nombre, telefono: (e.telefono || "").trim(), pais: "", nivel: "Tiepido", estado: "Da contattare",
  ...
  App.render();
  const partes = [];
  if (agregados) partes.push(agregados + (agregados === 1 ? " contatto importato" : " contatti importati"));
  if (duplicados) partes.push(duplicados + " " + (duplicados === 1 ? "era" : "erano") + " già nella tua lista");
  App.showToast(partes.length ? partes.join(" · ") : "Nessun nuovo contatto importato");
}
...
const Actions = {
  ...
  // Gruppo WhatsApp di supporto dello sponsor — diverso dalla chat diretta:
  // serve affinché un nuovo socio possa chiedere aiuto lì quando ha già
  // superato l'ora di controllo concordata e il suo sponsor non è disponibile
  // in quel momento.
  ...
  // Importa contatti: nei browser con supporto (Chrome Android) apre il
  // selettore nativo del telefono direttamente; negli altri (iPhone, computer)
  // ricade sul modal "incolla lista" come alternativa.
  "importar-contactos": async function () {
    if (navigator.contacts && navigator.contacts.select) {
      ...
      try {
        ...
        return; // l'utente ha annullato il selettore, o il browser lo ha bloccato
      }
  ...
  // Registra una chiamata o un messaggio fatto a un contatto della Lista dei 250: somma al
  // contatore del giorno (Report Settimanale → "Oggi"), fa avanzare il contatto a "Contattato"
  // se era ancora "Da contattare", e se aveva un follow-up in sospeso lo segna come fatto.
  // Così il lavoro reale con la lista si riflette davvero nel report da condividere.
  "registrar-contacto": function (arg, el) {
    ...
    App.persist(true);
    App.showToast(tipo === "mensajes" ? "Messaggio registrato" : "Chiamata registrata");
    App.render();
  },
  ...
  "delete-cliente": function (arg) {
    ...
    App.ui.clienteEditId = null;
    App.persist(true);
    App.showToast("Cliente eliminato");
    App.render();
  },
