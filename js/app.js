  ...
  bindEvents() {
    ...
    root.addEventListener("input", (e) => {
      ...
      if (el.dataset && el.dataset.field) {
        ...
        // formularios con borrador (contact / activitate din agendă / zoom / persoană din Întâlnirea de Focalizare / ascendent / S.O.S. / contact din eveniment / sponsor din FAB / partener / client): nici acestea nu se re-randează, ca să nu se piardă focusul
...
const Actions = {
  ...
  // Înregistrează un apel sau un mesaj făcut către un contact din Lista de 250: adaugă la
  // contorul zilei (Raport Săptămânal → "Astăzi"), avansează contactul la "Contactat" dacă
  // era încă "De contactat", și dacă avea o urmărire programată o marchează ca făcută.
  // Astfel, munca reală cu lista chiar apare reflectată în raportul de împărtășit.
  "registrar-contacto": function (arg, el) {
    ...
    App.persist(true);
    App.showToast(tipo === "mensajes" ? "Mesaj înregistrat" : "Apel înregistrat");
    App.render();
  },
