/* ---------------------------------------------------------------
   APP CONTROLLER — state, render loop, events
--------------------------------------------------------------- */
...
/* Instagram, TikTok, and YouTube don't have a public URL to prefill text
   with (unlike WhatsApp/Facebook/LinkedIn) — that's why for those platforms
   the button copies the message to the clipboard and opens the web/app,
   instead of faking an intent those networks don't offer. YouTube isn't
   among the options because it doesn't support posting loose text like this. */
...
const App = {
  ...
  checkAgendaAlarmas() {
    ...
    (dia.actividades || []).forEach((a) => {
      ...
      revisar(a, "Cumbre 90 — " + tipo.label, a.nota || "You have this scheduled at " + a.hora + ".");
    });
    (dia.zooms || []).forEach((z) => {
      revisar(z, "Cumbre 90 — Zoom: " + (z.titulo || "Meeting"), "Starts at " + z.hora + ".");
    });
  },
  ...
  render() {
    ...
    document.getElementById("fab-slot").innerHTML = isAuth
      ? '<button class="fab-whatsapp" data-action="abrir-whatsapp-fab" title="Message your sponsor">' + Icon("message-circle", { size: 24, color: "#fff" }) + "</button>" +
        (!ui.tourAbierto
          ? '<button class="fab-tour" data-action="iniciar-tour" title="See the guided tour">' + Icon("compass", { size: 22, color: "#fff" }) + "</button>"
          : "") +
        (hayCumpleanosHoy
          ? '<button class="fab-birthday" data-action="open-cumpleanos" title="Today\'s birthdays">' + Icon("party", { size: 20, color: "#fff" }) + "</button>"
          : "") +
        (grupoWhatsapp && grupoWhatsapp.trim()
          ? '<button class="fab-whatsapp-group" data-action="abrir-whatsapp-grupo-fab" title="Support WhatsApp group">' + Icon("users", { size: 20, color: "#fff" }) + "</button>"
          : "")
      : "";
  ...
  bindEvents() {
    ...
    // text fields (data-field): update state without re-rendering, so focus isn't lost
    root.addEventListener("input", (e) => {
      ...
      if (el.dataset && el.dataset.field) {
        ...
        // forms with a draft (...): also skip re-rendering, so focus isn't lost
        ...
        // contact search filter: applied straight to the DOM, without going through render()
        ...
        // customer search filter: applied straight to the DOM, without going through render()
        const q = el.value.trim().toLowerCase();
        ...
        // product search filter: applied straight to the DOM, without going through render()
        const q = el.value.trim().toLowerCase();
    ...
    // file inputs (photos): data-target points to a state path, or the special __onboardingFoto prefix
    root.addEventListener("change", (e) => {
      ...
      if (el.tagName === "INPUT" && el.dataset && el.dataset.field && (el.dataset.field.indexOf("comprasQuincena.") === 0 || el.dataset.field.indexOf("catalogoProductos.") === 0)) {
        // recalculates the product calculator's totals when leaving the field (not on every keystroke, so focus isn't lost)
        this.render();
        return;
      }
      ...
      if (el.type === "file" && el.dataset && el.dataset.target === "__importBackup") {
        ...
        reader.onload = () => {
          ...
          try {
            ...
            this.showToast("That file isn't a valid Cumbre 90 backup.");
            return;
          }
          if (!window.confirm("This will replace all your current data (List of 250, Genealogy Tree, progress) with the data from the backup file. Continue?")) return;
...
// Adds new contacts to the List of 250 from {nombre, telefono}, without
// duplicating by phone (used both by the native picker and by the "paste
// list" fallback modal for "Import contacts").
function importarContactosDesdeListado(entradas) {
  ...
  entradas.forEach(function (e) {
    ...
    App.state.contactos.push({
      ...
      nombre: nombre, telefono: (e.telefono || "").trim(), pais: "", nivel: "Warm", estado: "To contact",
  ...
  App.render();
  const partes = [];
  if (agregados) partes.push(agregados + " contact" + (agregados === 1 ? "" : "s") + " imported");
  if (duplicados) partes.push(duplicados + " " + (duplicados === 1 ? "was" : "were") + " already in your list");
  App.showToast(partes.length ? partes.join(" · ") : "No new contacts were imported");
}
...
const Actions = {
  ...
  // Sponsor's support WhatsApp group — different from the direct chat: it's
  // where a new partner can ask for help when they're past their agreed
  // check-in time and their sponsor isn't available right then.
  ...
  "tour-saltar": function () {
    ...
    App.state.tourVisto = true;
    App.persist(true);
    App.showToast("You can watch the tour again anytime using the floating button");
    App.render();
  },
  ...
  "salir-app": function () {
    ...
    App.render();
    window.close();
    App.showToast("If it didn't close on its own, you can now close this tab or go back.");
  },
  ...
  /* -------- "Share" modal for image cards (WhatsApp/Instagram/TikTok/Facebook/LinkedIn/YouTube) -------- */
  ...
  "compartir-imagen-descargar": function () {
    ...
    App.showToast("The image was downloaded — you can now attach it wherever you want to share it.");
    App.ui.compartirImagenDraft = null;
    App.render();
  },
  ...
  "compartir-imagen-mas-opciones": function () {
    ...
    if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
      navigator.share({ files: [file], title: "Cumbre 90", text: d.shareText }).catch(function (err) {
        ...
        downloadBlob(d.blob, d.filename);
        App.showToast("Couldn't open the share panel — the image was downloaded instead.");
      });
    } else {
      downloadBlob(d.blob, d.filename);
      App.showToast("The image was downloaded — you can now attach it wherever you want to share it.");
  ...
  /* Instagram, TikTok, and YouTube don't accept receiving an attached file
     from a web page without its own server — the image is downloaded and
     the app/site is opened so the partner can attach it by hand, always
     letting them know with a toast. */
  "compartir-imagen-plataforma": function (arg) {
    ...
    App.showToast("The image was downloaded — open it in " + (labels[arg] || arg) + " and attach it there.");
    App.ui.compartirImagenDraft = null;
    App.render();
  },
  ...
  "descargar-respaldo": function () {
    ...
    const fecha = hoyISO();
    downloadBlob(blob, "Cumbre90-Backup-" + slugFile(App.state.nombre || "partner") + "-" + fecha + ".json");
  ...
  "marcar-lema-hoy": function () {
    ...
    if (foco.racha === 7) {
      App.showToast("1 week straight living \"" + pilar.t + "\"! 🔥");
    } else if (foco.racha === 21) {
      App.celebrate();
      App.ui.logro = { titulo: pilar.t, sub: "21 days straight — now it's a habit.", tipo: "generic" };
      App.addActividad("You turned \"" + pilar.t + "\" into a 21-day habit.");
    }
    App.persist(true);
    App.render();
  },
  ...
  "share-logro-plataforma": function (arg) {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(text).then(
        () => App.showToast("Text copied — paste it in your story or post."),
        () => App.showToast("Couldn't copy the text.")
      );
    } else {
      App.showToast("Couldn't copy the text.");
    }
  ...
  "save-contacto": function () {
    ...
    if (App.ui.contactoEditId) {
      ...
      if (idx !== -1) {
        ...
        // estadoFecha is only touched if the status actually changed on save —
        // that way editing another field (e.g. a note) doesn't reset the status date.
    ...
    const quedoComoSocio = d.estado === "Partner" && estadoAnterior !== "Partner";
    App.ui.contactoDraft = null;
    App.ui.contactoEditId = null;
    App.persist(true);
  ...
  // Import contacts: on browsers with support (Chrome Android) opens the
  // phone's native picker directly; on the rest (iPhone, desktop) falls
  // back to the "paste list" modal.
  "importar-contactos": async function () {
    if (navigator.contacts && navigator.contacts.select) {
      ...
      try {
        ...
        return; // user cancelled the picker, or the browser blocked it
      }
  ...
  // Logs a call or message made to a List of 250 contact: adds to the daily
  // counter (Weekly Report → "Today"), advances the contact to "Contacted" if
  // it was still "To contact", and if it had a pending follow-up marks it done.
  // This way real work on the list actually shows up in the report you share.
  "registrar-contacto": function (arg, el) {
    ...
    App.persist(true);
    App.showToast(tipo === "mensajes" ? "Message logged" : "Call logged");
    App.render();
  },
  ...
  "marcar-seguimiento-hecho": function (arg) {
    ...
    c.proximoSeguimiento = null;
    App.persist(true);
    App.showToast("Follow-up marked as done");
    App.render();
  },
    ...
    for (let i = 0; i < 6; i++) {
      ...
      App.state.agenda[weekdayId].actividades.push(
        Object.assign(nuevaActividadAgenda(), {
          tipo: "plan6",
          hora: hora,
          nota: "Day " + (i + 1) + " — " + diaInfo.titulo + " — with " + d.contactoNombre,
          recordar: !!hora,
          recordarMin: 10,
        })
      );
  ...
  "delete-distribuidor-duplica": function (arg) {
    ...
    App.persist(true);
    App.showToast("Distributor deleted");
    App.render();
  },
  ...
  "share-paso-reflexion": function (arg) {
    ...
    if (navigator.share) {
      ...
      navigator.clipboard.writeText(texto).then(
        () => App.showToast("Reflection copied — paste it anywhere you like!"),
        () => App.showToast("Couldn't copy the text")
      );
    } else {
      App.showToast("Couldn't copy the text");
  ...
  "set-escenario-avance": function (arg, el) {
    ...
    if (completo && !App.state.escenarioCompletado) {
      ...
      App.celebrate();
      App.ui.logro = { titulo: "Life Scenario", sub: "You connected the 8 points into a perfect circle — now you're clear on your “why.”", tipo: "generic" };
      App.addActividad("Completed your Life Scenario — perfect circle!");
    } else if (!completo) {
  ...
  "delete-persona-enfoque": function (arg, el) {
    ...
    App.ui.personaEnfoqueDraft = null;
    App.persist(true);
    App.showToast("Person deleted");
    App.render();
  },

  /* -------- My Genealogy Tree — upline -------- */
  ...
  "add-producto": function () {
    const catalogo = getCatalogoProductos(App.state, App.state.pais || "CO");
    catalogo.push(nuevoProductoCatalogo({ categoria: "My products" }));
    App.persist(true);
    App.render();
  },
