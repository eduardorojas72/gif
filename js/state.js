...
function nuevoProductoCatalogo(seed) {
  ...
  return {
    ...
    categoria: seed.categoria || "I miei prodotti",
    nombre: seed.nombre || "",
    pv: seed.pv || 0,
...
function emptyCatalogoProductosPais(paisId) {
  ...
  const lineasLibres = Array.from({ length: 20 }, function (_, i) {
    return nuevoProductoCatalogo({ id: paisId + "-blank" + i, categoria: "I miei prodotti" });
  });
...
function emptyArbolGenealogico() {
  return {
    ...
    patrocinador: { nombre: "", atomyId: "", rango: "", pais: "", telefono: "", zoomId: "", zoomContrasena: "", horarioNoLlamar: "", grupoWhatsapp: "" },
    ascendentes: [],
  };
}
...
/* Somma mesi di calendario a una data ISO (per il follow-up a "11 mesi" dei Clienti). */
...
/* Clienti il cui compleanno (giorno e mese) è oggi — per l'avviso flottante. */
...
const calMesesEs = ["gennaio", "febbraio", "marzo", "aprile", "maggio", "giugno", "luglio", "agosto", "settembre", "ottobre", "novembre", "dicembre"];
...
function calQuincenaLabel(key) {
  const { year, month, half } = calParseQuincenaKey(key);
  return (half === 1 ? "1–15" : "16–fine") + " " + calMesesEs[month - 1] + " " + year;
}
...
function hydrateState(parsed) {
  ...
  merged.contactos = Array.isArray(parsed.contactos)
    ? parsed.contactos.map((c) => {
        const mc = Object.assign(
          { id: "c" + Math.random().toString(36).slice(2, 9), nombre: "", telefono: "", pais: "", nivel: "Tiepido", estado: "Da contattare", notas: "", notaSeguimiento: "", proximoSeguimiento: null, creado: hoyISO(), seguimientosRealizados: [] },
          c
        );
  ...
  merged.idiomaInforme = IDIOMAS_INFORME.some(function (i) { return i.id === parsed.idiomaInforme; }) ? parsed.idiomaInforme : "it";
  ...
  merged.arbolGenealogico = {
    ...
    patrocinador: Object.assign(
      { nombre: "", atomyId: "", rango: "", pais: "", telefono: "", zoomId: "", zoomContrasena: "", horarioNoLlamar: "", grupoWhatsapp: "" },
