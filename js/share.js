...
function downloadCertificado(state) {
  const t = themeColors();
  const fecha = new Date().toLocaleDateString("it-IT", { day: "2-digit", month: "long", year: "numeric" });
  const nombre = safeXml(state.nombre || "Partner Atomy");
  ...
  const svg =
    ...
    '<text x="450" y="230" text-anchor="middle" font-family="Georgia, serif" font-size="42" fill="' + t.text + '">Certificato di Vetta</text>' +
    '<text x="450" y="300" text-anchor="middle" font-family="Arial" font-size="16" fill="' + t.textSoft + '">Conferito a</text>' +
    '<text x="450" y="350" text-anchor="middle" font-family="Georgia, serif" font-size="34" fill="' + t.gold + '">' + nombre + "</text>" +
    '<text x="450" y="400" text-anchor="middle" font-family="Arial" font-size="15" fill="' + t.textSoft + '">per aver completato il Piano di 90 Giorni e raggiunto il rango di Sales Master</text>' +
...
function downloadDiaCard(state, diaId) {
  ...
  const lines = entradas.length
    ...
    : '<text x="60" y="' + y + '" font-family="Arial" font-size="15" fill="' + t.textSoft + '">Non hai ancora compilato i campi di questa tappa.</text>';

  const svg =
    '<svg xmlns="http://www.w3.org/2000/svg" width="' + width + '" height="' + height + '">' +
    '<rect width="' + width + '" height="' + height + '" fill="' + t.bg + '"/>' +
    '<rect x="20" y="20" width="' + (width - 40) + '" height="' + (height - 40) + '" rx="20" fill="none" stroke="' + t.gold + '" stroke-width="2.5"/>' +
    '<text x="60" y="90" font-family="Arial" font-size="13" letter-spacing="4" font-weight="bold" fill="' + t.gold + '">CUMBRE 90 · TAPPA ' + dia.id + "</text>" +
