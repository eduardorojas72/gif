...
function svgToPngDownload(svgMarkup, width, height, filename) {
  ...
  img.onerror = function () {
    URL.revokeObjectURL(url);
    App.showToast("Impossible de générer l'image. Réessaie.");
  };
  img.src = url;
}
...
function downloadRecogCard(state) {
  ...
  svgToPngShare(svg, 800, 1000, filename, "Voici ma progression sur mon chemin vers Sales Master avec Atomy ! 🚀");
}
...
function downloadConsumidorVipCard(state) {
  ...
  fetch(template.img)
    ...
    .catch(function () {
      App.showToast("Impossible de charger l'image. Réessaie.");
    });
}
...
function downloadCertificado(state) {
  ...
  const nombre = safeXml(state.nombre || "Partenaire Atomy");
  ...
  const svg =
    ...
    '<text x="450" y="230" text-anchor="middle" font-family="Georgia, serif" font-size="42" fill="' + t.text + '">Certificat de Sommet</text>' +
    '<text x="450" y="300" text-anchor="middle" font-family="Arial" font-size="16" fill="' + t.textSoft + '">Décerné à</text>' +
    '<text x="450" y="350" text-anchor="middle" font-family="Georgia, serif" font-size="34" fill="' + t.gold + '">' + nombre + "</text>" +
    '<text x="450" y="400" text-anchor="middle" font-family="Arial" font-size="15" fill="' + t.textSoft + '">pour avoir terminé le Plan de 90 Jours et atteint le rang de Sales Master</text>' +
    ...
    "</svg>";
  svgToPngDownload(svg, 900, 620, "Certificat-Cumbre90-" + slugFile(state.nombre || "partenaire") + ".png");
}
...
function downloadDiaCard(state, diaId) {
  ...
  const lines = entradas.length
    ...
    : '<text x="60" y="' + y + '" font-family="Arial" font-size="15" fill="' + t.textSoft + '">Tu n\'as pas encore rempli les champs de cette étape.</text>';

  const svg =
    '<svg xmlns="http://www.w3.org/2000/svg" width="' + width + '" height="' + height + '">' +
    '<rect width="' + width + '" height="' + height + '" fill="' + t.bg + '"/>' +
    '<rect x="20" y="20" width="' + (width - 40) + '" height="' + (height - 40) + '" rx="20" fill="none" stroke="' + t.gold + '" stroke-width="2.5"/>' +
    '<text x="60" y="90" font-family="Arial" font-size="13" letter-spacing="4" font-weight="bold" fill="' + t.gold + '">CUMBRE 90 · ÉTAPE ' + dia.id + "</text>" +
