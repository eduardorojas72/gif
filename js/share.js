...
function downloadCertificado(state) {
  const t = themeColors();
  const fecha = new Date().toLocaleDateString("pt-BR", { day: "2-digit", month: "long", year: "numeric" });
  const nombre = safeXml(state.nombre || "Sócio Atomy");
  ...
  const svg =
    ...
    '<text x="450" y="300" text-anchor="middle" font-family="Arial" font-size="16" fill="' + t.textSoft + '">Concedido a</text>' +
    '<text x="450" y="350" text-anchor="middle" font-family="Georgia, serif" font-size="34" fill="' + t.gold + '">' + nombre + "</text>" +
    '<text x="450" y="400" text-anchor="middle" font-family="Arial" font-size="15" fill="' + t.textSoft + '">por completar o Plano de 90 Dias e alcançar o rank de Sales Master</text>' +
...
function downloadDiaCard(state, diaId) {
  ...
  const lines = entradas.length
    ...
    : '<text x="60" y="' + y + '" font-family="Arial" font-size="15" fill="' + t.textSoft + '">Você ainda não completou os campos desta etapa.</text>';
  ...
  const svg =
    ...
    lines +
    '<text x="60" y="' + (height - 40) + '" font-family="Arial" font-size="12" fill="' + t.textSoft + '">Jornada rumo ao sucesso com a Atomy · ' + safeXml(state.nombre || "") + "</text>" +
    "</svg>";

  svgToPngShare(svg, width, height, "Cumbre90-Etapa" + dia.id + "-" + slugFile(dia.etapa) + ".png", "Avançando no meu Plano de 6 Dias com a Atomy! 🚀");
}
